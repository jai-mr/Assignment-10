...
def mis(model, device, test_loader, nimage = 64, dataset_used= 'CIFAR10'):
    ...
    dirName = gdrivepath + '/savedmodel/mis_class/'
    subdir = gdrivepath + '/savedmodel/mis_class/images'
    ...
    for index in range(0, nimage):
        ...
        path = f'gdrive/My Drive/Colab Notebooks/S10/savedmodel/mis_class/images/mis_{index+10}.png'
...
def gen_cam(model, layer, hm_lay= 0.5, img_lay= 0.5, dataset_used= 'CIFAR10' ,class_idx= None):
    ...
    if class_idx is None:
        dirName = gdrivepath + '/savedmodel/result_pred/'
        dir2Name = gdrivepath + '/savedmodel/heatmap_pred/'
        ...
        dirName = gdrivepath + '/savedmodel/result_act/'
        dir2Name = gdrivepath + '/savedmodel/heatmap_act/'
        if not os.path.exists(dirName) or not os.path.exists(dirName):
            os.mkdir(dirName)
            os.mkdir(dir2Name)
            print("Directory " , dirName ,  " Created ")
    ...
    dataset = ImageFolder(root=gdrivepath+'/savedmodel/mis_class/', transform=transform)
    ...
    for index, batch in enumerate(dataloader):
      ...
      if class_idx is None:
        fp_path_heat = f'gdrive/My Drive/Colab Notebooks/S10/savedmodel/heatmap_pred/map{index+10}_{layer}.png'
        save_image(heatmap, fp=fp_path_heat)
        fp_path_res = f'gdrive/My Drive/Colab Notebooks/S10/savedmodel/result_pred/result{index+10}_{layer}.png'
        save_image(result, fp=fp_path_res)
      else:
        fp_path_heat = f'gdrive/My Drive/Colab Notebooks/S10/savedmodel/heatmap_act/map{index+10}_{layer}.png'
        save_image(heatmap, fp=fp_path_heat)
        fp_path_res = f'gdrive/My Drive/Colab Notebooks/S10/savedmodel/result_act/result{index+10}_{layer}.png'
        save_image(result, fp=fp_path_res)
...
def plot_pred_cam(n,l):
  ...
  for index in range(0, n):
      ...
      path = gdrivepath + '/savedmodel/mis_class/images/mis_' + str(index+10) + '.png'
      img = plt.imread(path)
      axes[index, 1].imshow(img, interpolation= 'bilinear')
      axes[index, 1].axis('off')
      for layer in range(l):
        path = f'gdrive/My Drive/Colab Notebooks/S10/savedmodel/result_pred/result{index+10}_layer{layer+1}.png'
        img = plt.imread(path)
...
def plot_act_cam(n,l):
  ...
  for index in range(0, n):
      ...
      axes[index, 1].axis('off')
      for layer in range(l):
        path = f'gdrive/My Drive/Colab Notebooks/S10/savedmodel/result_act/result{index+10}_layer{layer+1}.png'
        img = plt.imread(path)
        axes[index, layer+2].imshow(img.squeeze(), cmap='gray_r', interpolation= 'bilinear')
        axes[index, layer+2].set_title(f'Layer: {layer+1}')
        axes[index, layer+2].axis('off')
...
def plot_pred_heatmap(n,l):
  ...
  for index in range(0, n):
      ...
      axes[index, 1].axis('off')
      for layer in range(l):
        path = f'gdrive/My Drive/Colab Notebooks/S10/savedmodel/heatmap_pred/heatmap{index+10}_layer{layer+1}.png'
        img = plt.imread(path)
        axes[index, layer+2].imshow(img.squeeze(), cmap='gray_r', interpolation= 'bilinear')
        axes[index, layer+2].set_title(f'Layer: {layer+1}')
        axes[index, layer+2].axis('off')
...
def plot_act_heatmap(n,l):
  ...
  for index in range(0, n):
      ...
      axes[index, 1].axis('off')
      for layer in range(l):
        path = f'gdrive/My Drive/Colab Notebooks/S10/savedmodel/heatmap_act/heatmap{index+10}_layer{layer+1}.png'
        img = plt.imread(path)
        axes[index, layer+2].imshow(img.squeeze(), cmap='gray_r', interpolation= 'bilinear')
        axes[index, layer+2].set_title(f'Layer: {layer+1}')
        axes[index, layer+2].axis('off')
